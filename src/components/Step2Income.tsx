...
import { Briefcase, Home, TrendingUp, Wallet, Factory, ArrowLeft, ArrowRight, Activity, Percent } from 'lucide-react';
    ...
    <div id="s2">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card">
        ...
        <Accordion 
          ...
          title={<span style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#fff' }}><Briefcase size={18} /> SALARY HEAD</span>}
        >
          <div style={{background:'linear-gradient(135deg,#0A1628,#1A6FD8)', margin: '-20px -20px 20px -20px', padding: '16px 20px', color: '#fff'}}>
          ...
          <div className="g3" style={{ marginBottom: '6px' }}>
            <Field label="HRA Received" name="iHRA" />
            <Field label="Annual Rent Paid" name="iRent" />
            <SelectField label="City Type" name="iCity" options={[{ label: 'Metro', value: 'metro' }, { label: 'Non-Metro', value: 'nonmetro' }]} />
          </div>
          <div className="g2" style={{ marginBottom: '10px' }}>
            <Field label="Landlord PAN (> ₹1L/yr)" name="iLandlordPAN" type="text" maxLength={10} onInputTransform={v => v.toUpperCase()} />
          ...
          <div className="g4" style={{ marginBottom: '10px' }}>
            ...
            <Field label="Other Allowances" name="iOthAllow" />
            <Field label="Overtime Allowance" name="iOvertime" />
            <Field label="Project Allowance" name="iDeputation" />
          </div>
          ...
          <div style={{ background: 'linear-gradient(135deg,#0A1628,#0F2044)', borderRadius: '10px', padding: '16px', marginTop: '4px' }}>
            <div style={{ color: '#93C5FD', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}><Activity size={14} /> Live Salary Computation</div>
        ...
        <Accordion theme="default"
          ...
          title={<span style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#fff' }}><Home size={18} /> HOUSE PROPERTY HEAD</span>}
        >
          <div style={{background:'linear-gradient(135deg,#065F46,#059669)', margin: '-20px -20px 20px -20px', padding: '16px 20px', color: '#fff'}}>
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '14px', flexWrap: 'wrap' }}>
            ...
            <button className={`rtab ${values.hpType === 'lo' ? 'on' : ''}`} onClick={() => handleChange('hpType', 'lo')}>Let-Out / Deemed</button>
            <button className={`rtab ${values.hpType === 'both' ? 'on' : ''}`} onClick={() => handleChange('hpType', 'both')}>Both Properties</button>
          </div>

          {(values.hpType === 'sop' || values.hpType === 'both') && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} style={{ background: 'var(--blue-light)', border: '1px solid var(--blue-mid)', borderRadius: '9px', padding: '14px', marginBottom: '12px', overflow: 'hidden' }}>
              ...
              <div className="g3">
                ...
                <Field label="Home Loan Interest (24b)" name="iHP_SOPInt" hint="Max: Rs.2,00,000 (Old Regime)" />
                <Field label="Pre-Construction Int." name="iHP_PreConInt" />
              </div>
            </motion.div>
          )}

          {(values.hpType === 'lo' || values.hpType === 'both') && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} style={{ background: 'var(--green-light)', border: '1px solid #A7F3D0', borderRadius: '9px', padding: '14px', marginBottom: '12px', overflow: 'hidden' }}>
          ...
          <div className="g2">
            <Field label="HP Override (if known)" name="iHP" hint="Leave blank to use calculator" />
          </div>
        </Accordion>
        ...
        <Accordion theme="default"
          ...
          title={<span style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#fff' }}><TrendingUp size={18} /> CAPITAL GAINS</span>}
        >
          <div style={{background:'linear-gradient(135deg,#1e3a5f,#1D4ED8)', margin: '-20px -20px 20px -20px', padding: '16px 20px', color: '#fff'}}>
          ...
          <div className="cg-type-grid">
            <div className="cg-type-card stcg111a">
              ...
              <div className="cg-input-row">
                <Field label="Sale Consider" name="iSTCG_sale" />
                <Field label="Cost Acq" name="iSTCG_cost" />
                <Field label="Transfer Exp" name="iSTCG_exp" />
              </div>
            </div>

            <div className="cg-type-card stcg-other">
              <div className="cg-type-badge">📉 Other STCG</div>
              <div className="cg-type-title">Other Assets (Slab)</div>
              <div className="cg-input-row">
                <Field label="Sale Consider" name="iSTCGOth_sale" />
                <Field label="Cost Acq" name="iSTCGOth_cost" />
                <Field label="Transfer Exp" name="iSTCGOth_exp" />
              </div>
            </div>
            ...
            <div className="cg-type-card ltcg112a">
              ...
              <div className="cg-input-row">
                <Field label="Sale Consider" name="iLTCG_sale" />
                <Field label="Cost Acq" name="iLTCG_cost" />
                <Field label="Transfer Exp" name="iLTCG_exp" />
              </div>
            </div>
            ...
            <div className="cg-type-card ltcg-other">
              ...
              <div className="cg-input-row">
                <Field label="Sale Consider" name="iLTCGOth_sale" />
                ...
                <Field label="Transfer Exp" name="iLTCGOth_exp" />
              </div>
            </div>
          </div>
          ...
          <div style={{ background: 'var(--green-light)', border: '1.5px solid #A7F3D0', borderRadius: '11px', padding: '16px', marginBottom: '16px' }}>
              ...
              <div className="g3">
                <Field label="Sec 54 (Res. House)" name="ex54" />
                <Field label="Sec 54EC (NHAI / REC)" name="ex54EC" max={5000000} />
                <Field label="Sec 54F (Any LTCG)" name="ex54F" />
              </div>
          </div>
        </Accordion>
        ...
        <Accordion theme="default"
          ...
          title={<span style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#fff' }}><Wallet size={18} /> OTHER SOURCES</span>}
        >
          <div style={{background:'linear-gradient(135deg,#0891B2,#0E7490)', margin: '-20px -20px 20px -20px', padding: '16px 20px', color: '#fff'}}>
          ...
          <div className="g3">
            ...
            <Field label="Lottery / Games" name="iLotteryOS" hint="Taxed @ 30% u/s 115BB" />
            <Field label="Family Pension" name="iFamPen" />
            <Field label="Misc. Income" name="iOth" />
          </div>
        </Accordion>
        ...
        <Accordion theme="default"
          ...
          title={<span style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#fff' }}><Factory size={18} /> BUSINESS / PROFESSION</span>}
        >
          <div style={{background:'linear-gradient(135deg,#7C2D12,#EA580C)', margin: '-20px -20px 20px -20px', padding: '16px 20px', color: '#fff'}}>
              <span style={{ fontSize: '12px', fontWeight: 600 }}>PGBP (u/s 28–44)</span>
          </div>

          <div style={{ display: 'flex', gap: '8px', marginBottom: '14px', flexWrap: 'wrap' }}>
            <button className={`rtab ${values.pgbpScheme === '44AD' ? 'on' : ''}`} onClick={() => handleChange('pgbpScheme', '44AD')}><Percent size={14} style={{ display: 'inline-flex', verticalAlign: 'text-bottom' }} /> 44AD (Business)</button>
            <button className={`rtab ${values.pgbpScheme === '44ADA' ? 'on' : ''}`} onClick={() => handleChange('pgbpScheme', '44ADA')}><Percent size={14} style={{ display: 'inline-flex', verticalAlign: 'text-bottom' }} /> 44ADA (Profession)</button>
          ...
          <motion.div layout>
            {values.pgbpScheme === '44AD' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="g3">
                <Field label="Gross Turnover" name="i44AD_turnover" />
                <Field label="Cash Receipts" name="i44AD_cash" hint="Cash @8%, Digital @6%" />
                <SelectField label="Rate" name="i44AD_rate" options={[{label: '8%', value: '8'}, {label: '6%', value: '6'}, {label: 'Mixed', value: 'mixed'}]} />
              </motion.div>
            )}

            {values.pgbpScheme === '44ADA' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="g2">
                <Field label="Gross Professional Receipts" name="i44ADA_receipts" />
                <Field label="Actual Income Declared" name="i44ADA_actual" />
              </motion.div>
            )}
            ...
            {values.pgbpScheme === 'regular' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="g3">
                ...
                <Field label="Cost of Goods/Exp" name="iPGBP_cogs" />
        ...
        <div className="section-label">TDS / Advance Tax Paid</div>
        <div className="g3">
        ...
        <div className="btn-row">
          ...
          <button className="btn-primary" onClick={() => setStep(3)}>Next: Deductions <ArrowRight size={16} /></button>
        </div>
      </motion.div>
    </div>
  );
